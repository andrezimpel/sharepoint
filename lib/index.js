...
class Sharepoint {
  ...
  async getFileData ({ path, fileName, responseType="stream" }) {
    this.checkHeaders()
  ...
  async createFile (options) {
    ...
    const { path, fileName, data, overwrite=true } = options
    ...
    await axios({
      method: 'post',
      url: `${this.url}/_api/web/GetFolderByServerRelativeUrl('${this.site.serverRelativeUrl}${path}')/Files/add(url='${fileName}', Overwrite=${overwrite})`,
      data,
      headers: {
        ...this.headers,
        'X-RequestDigest': formDigestValue
